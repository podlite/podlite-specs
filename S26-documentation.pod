...
(POD) markup. Compared to POD, Raku Pod is much more
...
An identifier composed of an alphabetical character followed by 
...
Every Pod block may be declared in any of three forms:
...
This option specifies that the block may be given an associated description or title using the
C<:caption> option.
...
within Markdown files, Markdown itself doesn't have formal named blocks like the Pod syntax. 
Instead, Markdown uses a lightweight markup to structure content. The above table provides a mapping
between Pod block names and their corresponding Markdown elements.

=begin table :nested :caption("A table of correspondences for some Pod blocks and markdown blocks")

    Pod                                             Markdown                Describtion
...
In addition to the ordered list Pod also supports 
...
For example:
=begin code :allow<B> 
 =item B<[ ]> Buy groceries
 =itme B<[x]> Clean the garage 
=end code
...
=begin code
 =begin table
    ...
    Programming Language        Possible Values 
...
D<Pod comments|Pod comment> refers to Pod blocks that should not
be displayed by any renderer. However, they are still may be part of the
internal Pod representation. However, they can still be part of internal
Pod representation.
...
Pod allows to include images using a C<=picture> block with an inline version
...
indicated using different schemas: C<https:> for remote images or C<file:> for local ones.
...
Pod includes built-in support for Markdown, making it easy to include Markdown
content within Pod documents. This is achieved using the C<=markdown> block.
...
This block allows to seamlessly combine Pod's structured blocks with
...
within the contents of (most types of) Pod block. Formatting codes are
...
Pod provides two formatting codes that allow positioning characters or words.

=begin item
To create subscript text, you can use the C<J<>> formatting code, where 'J' represents 
"Junior text." Subscript positions one or more characters slightly below the normal
line of type.

 HJ<2>O

This code renders as "H<sub>2</sub>O," which signifies that the '2' is positioned 
slightly below the baseline, as is often seen in chemical formulas like H₂O (water).
=end item

=begin item
To create superscript text, it possible to use the C<H<>> formatting code, where 'H'
stands for "High text." Superscript positions one or more characters slightly
above the normal line of type

 XH<2>

This code renders as "X<sup>2</sup>," indicating that the '2' is positioned 
slightly above the baseline. This is commonly used for mathematical exponents, as in X².
=end item
...
Pod include the possibility of assigning a unique identifier to a block.
...
You can use the following URI forms in a
placement link:

=item C<http:> and C<https:>
=item C<file:>

For L<images|#Inserting Pictures> C<P<>> works as inline equivalent of a C<=picture> block
in the following form: C<P<alternative text|source of the image>>.

=head3 Contextual backlinks

Pod introduces the concept of contextual backward links C<W<>> (or contextual backlinks),
...
=head3 Backward links

Bidirectional or backward linking refers to the ability to navigate
from a destination page back to the source page through
a hyperlink or other means.

Usual link are works by the following way:

=begin code :allow<B>
 C<L<doc:perldata>>   ->  Perl data page
   source                   destanation
=end code

Backward is working in anothe manner:

=begin code :allow<B>
  C<W<doc:perldata>>   <-  Perl data page   
  destanation               source             
=end code

Here are examples of bidirectional or backward linking:
...
A contextual backlink displays not only a reference 
from another location, but the specific context around that 
reference—for instance, the page of a book or the referencing 
paragraph.

Backward links operate similarly to standard links 
but might be distinguished by color.

Backward links may be highlighted by color, and it is essential 
to see the context surrounding the backlink to understand 
the implied meaning. Backlinks on the source page can be displayed 
with surrounding text chunks or placed at the end of the text, 
in the margin of the page, or highlighted in some other way.
...
with the colon-prefixed and postfix emoji shortname:
    ...
    Pod uses emoji codes to express emotions and actions,
...
to include emojis in your Pod document.
...
single C<E<>> code, separated by semicolons:
    ...
    Raku makes considerable use of E<LEFT DOUBLE ANGLE BRACKET;hellip;0xBB;:smile:>.
...
=para
E<dagger> The Raku C<for> loop is far more powerful than its Perl 5
predecessor.
=end nested
...
=begin table :nested
    ...
    C<O<...>>            Strikethrough
    C<P<...>>            Placement link, inline eqivalent for C<=picture> block
    C<Q<>>               reserved
